...
const ALLOWED_DOMAINS = ['clientascension.com'];
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
            hd: 'clientascension.com' // Restrict to specific domain
          }
        }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!validateDomain(email)) {
        setError(`Only users with @${ALLOWED_DOMAINS.join(' or @')} email addresses can access this system.`);
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background to-muted/20 p-4">
      <Card className="w-full max-w-md mx-auto shadow-lg">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Sign in to Client Ascension Dashboard
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <FormWrapper id="email" label="Organization Email" required>
              <div className="relative">
                <Mail className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input
                  id="email"
                  type="email"
                  placeholder="Enter your @clientascension.com email"
                  value={email}
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Only @clientascension.com emails are allowed
              </p>
            </FormWrapper>
          ...
          <div className="text-center text-sm text-muted-foreground">
            ...
            <p className="mt-2 text-xs">
              Access restricted to Client Ascension organization members
            </p>
          </div>
        </CardContent>
      </Card>
