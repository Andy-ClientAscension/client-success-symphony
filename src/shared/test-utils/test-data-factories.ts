...
export function createMockClient(overrides: Partial<any> = {}) {
  return {
    ...
    status: faker.helpers.arrayElement(['new', 'active', 'at-risk', 'churned', 'paused', 'graduated']),
    startDate: faker.date.past().toISOString(),
    endDate: faker.date.future().toISOString(),
    contractValue: faker.number.int({ min: 1000, max: 100000 }),
    progress: faker.number.int({ min: 0, max: 100 }),
    csm: faker.person.firstName(),
    team: faker.helpers.arrayElement(['Team-Andy', 'Team-Chris', 'Team-Alex']),
    callsBooked: faker.number.int({ min: 0, max: 50 }),
    dealsClosed: faker.number.int({ min: 0, max: 20 }),
